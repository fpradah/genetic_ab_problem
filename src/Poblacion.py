...
class Poblacion:
    ...
    def getBestCromosomas(self):
        best_cromosomas = []
        for i in range(self.tamano):
            if self.poblacion[i].puntuacion > 0.9 and \
             self.poblacion[i].puntuacion < 1:
    ...
    def newGeneration(self):
        ...
        for pos_heredado in range(max_herencia):
        ...
        for pos_mutado in range(max_mutacion):
            if (pos_heredado >= len(best_cromosomas)):
        ...
        for pos_cruzado in range(max_cruce):
            if (pos_heredado >= len(best_cromosomas)):
